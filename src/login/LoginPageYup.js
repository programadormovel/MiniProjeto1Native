...
const LoginPageYup = ({ navigation }) => {

  // Definição do Schema Yup de Conversão e Validação de Dados
  const schema = yup.object().shape({
    email: yup.string().required('O e-mail é obrigatório!').email('Digite um e-mail válido!'),
    password: yup.string().required('A senha precisa ser definida!').min(6, 'Senha com no mínimo de 6 caracteres!')
  });
  ...
  const {
    ...
    handleSubmit,
    formState: { errors },
  } = useForm({ defaultValues: { email: "", password: "" }, validationSchema: schema });
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.container}>
        ...
        <Controller
          control={control}
          // rules={{ validate: (email) => schema.validate({ email }).catch((e) => {if(e.message.includes('mail')) e.message})}}
          name="email"
        ...
        <Controller
          control={control}
          // rules={{ validate: (password) => schema.validate({ password }).catch((e) => {if(e.message.includes('enha')) e.message}) }}
          name="password"
          render={({ field: { onChange, onBlur, value }, fieldState={error} }) => (
            <TextField
